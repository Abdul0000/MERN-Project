...
const Chats = () => {
  ...
  return (
    <div className="bg-slate-600 flex flex-col items-center w-full h-screen p-2 pb-6">
      <header className="m-6 flex items-baseline justify-between w-11/12">
        ...
        {isLogin ? (
          <div className="flex items-center justify-center gap-4 -pt-10">
            {email && <p className="text-gray-100">{email}</p>}
            <button
              onClick={handOnLogout}
              className="text-xl bg-white hover:bg-gray-200 text-gray-500 font-semibold w-24 h-11 rounded"
            >
              Log Out
            </button>
          </div>
        ...
        <div className="flex flex-col flex-grow overflow-y-auto p-4 space-y-6">
          {finalData.length > 0 ? (
            ...
            <div className="flex items-center justify-center text-4xl font-semibold text-gray-100 h-full">
              <span className="text-red-300">How</span>{" "}
              <p className="ml-2">can I help you !!!</p>
            </div>
          )}
      ...
      {signupPopup ? (
        <Signup setSignupPopup={setSignupPopup} setPopup={setPopup} />
      ) : (
        ""
      )}
    </div>
